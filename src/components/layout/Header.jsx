    ...
    <>
      <header className="sticky top-0 z-50 border-b border-brand-secondary/10 bg-brand-surface/20 backdrop-blur-2xl dark:border-white/10">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-5 py-4 sm:px-8 lg:px-10">
          <a href="#home" className="flex min-w-0 items-center gap-3 text-brand-secondary dark:text-brand-text">
            <img src={logoMark} alt="Bernoulli Automation logo" className="h-10 w-10 shrink-0" />
              ...
              <span className="absolute flex w-6 items-center justify-center overflow-hidden">
                <span
                  className={`h-[1.8px] w-1/2 rounded-full bg-current transition-transform duration-300 ${
                    open ? 'origin-right scale-x-0' : 'origin-right scale-x-100'
                  }`}
                />
                <span
                  className={`h-[1.8px] w-1/2 rounded-full bg-current transition-transform duration-300 ${
                    open ? 'origin-left scale-x-0' : 'origin-left scale-x-100'
                  }`}
                />
              </span>
              <span
      ...
      <div
        className={`fixed inset-0 z-40 bg-brand-surface dark:bg-brand-secondary transition-transform duration-500 md:hidden ${
          open ? 'origin-top scale-y-100' : 'pointer-events-none origin-top scale-y-0'
        }`}
      >
